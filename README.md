...
- **Vercel** — hosting y despliegue continuo del sitio estático.
...
La aplicación está desplegada en Vercel: **[https://<tu-proyecto>.vercel.app](https://gestion-inventario-acme.vercel.app/)**.

### Usuario de prueba

Para ingresar sin necesidad de registrar un usuario nuevo, puedes usar el usuario predeterminado:

| Identificación | Contraseña |
|---|---|
| `123456` | `admin123` |

> Este usuario ya está cargado en la base de datos de Firebase que consume el proyecto. Si despliegas tu propia instancia de Firebase, deberás crear este usuario manualmente (o cualquier otro) antes de poder iniciar sesión.

## Instrucciones de ejecución

Este proyecto no requiere instalación de dependencias ni proceso de build: es HTML, CSS y JavaScript puro.

### Opción 1: usar la demo desplegada

Entra directamente a la URL de Vercel indicada arriba e inicia sesión con el usuario de prueba.
...
1. Clona el repositorio:
   ...
   ```
2. Todas las rutas de CSS y JS en los archivos HTML son **relativas** (sin `/` inicial), por lo que el proyecto funciona igual en local, en un subdirectorio (como GitHub Pages) o en la raíz de un dominio (como Vercel). Aun así, se recomienda servirlo con un servidor local en vez de abrir los `.html` directamente con `file://`, ya que las peticiones `fetch` a Firebase pueden bloquearse por CORS bajo el protocolo `file://`. Por ejemplo, con la extensión **Live Server** de VS Code, o con:
   ```bash
   npx serve .
   ```
3. Abre `index.html` en el navegador (o la URL que indique tu servidor local).
4. Inicia sesión con el usuario predeterminado (`123456` / `admin123`), con un usuario previamente registrado, o crea uno directamente desde la consola de Firebase en la ruta `/user/usuarios.json` si es el primer uso.
